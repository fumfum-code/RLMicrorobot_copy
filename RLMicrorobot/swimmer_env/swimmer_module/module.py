...
class Swimmer():
    ...
    def __init__(self, swimmer_type_id=1):
        with open(f'swimmer_env/model/swimmer_type{swimmer_type_id}.pickle','rb') as f:
            swimmer_type = pickle.load(f)
        #print(swimmer_type)
        with open(f'swimmer_env/model/param.pickle','rb') as f2:
        ...
        self.output_path = param['OUTPUT_PATH'] + f'PPO_{swimmer_type_id}_trained.csv'
        self.reward_gain = param['reward_gain']
        self.step_counter = 0
        self.reward_prev = 0

        self.analyse_record = False
    ...
    def getObservation(self, vector):
        ...
        for i in range(self.numParticle-1):
            observation[i+2] = self.vector[i][i+1][0]
            observation[i+5] = self.vector[i][i+1][1]
    ...
    def sygmoid(self, x, a, b):
        s = 1.5/(1 + np.exp(-x+a/b)) - 0.5
        return s
    ...
    def step(self, action : np.ndarray):
        ...
        for itr in range(self.ONE_STEP_ITER):
            ...
            if itr % self.OUTPUT_ITER == 0 and self.is_record :
                self.output(self.position, self.output_path)
                if self.analyse_record:
                    self.analyse(self.unit_vector)
                    #self.displacement(self.Get_center_position())
        ...
        reward = self.sygmoid(reward, 1/2, 1/2) 
    ...
    def output(self,position : np.ndarray, path):
    ...
    def analyse(self, vector):
        angle_list = []
        for i in range(self.numParticle-2):
            angle = self.get_angle(vector[i+1][i], vector[i+1][i+2])
            angle_list.append(angle)
        path = 'data/swimmer_type_1/phase_angle.csv'
        dir_exist_flag = os.path.isfile(path)
        if dir_exist_flag:
            with open(path, 'a') as f:
                np.savetxt(f, angle_list, delimiter=',', fmt='%.4f')
        else: 
            np.savetxt(path, angle_list, delimiter=',', fmt='%.4f')
    

    def displacement(self, gpos: np.ndarray):
        g = gpos.reshape([1,3])
        path = 'data/swimmer_type_1/displacement.csv'
        dir_exist_flag = os.path.isfile(path)
        if dir_exist_flag:
            with open(path, 'a') as f:
                np.savetxt(f, g, delimiter=',', fmt='%.4f')
        else: 
            np.savetxt(path, g, delimiter=',', fmt='%.4f')
...
if __name__ == '__main__':
    swimmer = Swimmer()
    swimmer.get_vector(swimmer.position, swimmer.numParticle)
    swimmer.get_angle_spring()
    print(f'force angle spring : {swimmer.force_angle_spring}')
"""
